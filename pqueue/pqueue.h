...
namespace persistent_queue {
    ...
    template <typename T>
    class persistent_queue<T>::iterator {
    private:
        static constexpr int END_MARKER = -1;
        int head;
        int tail;
        int pos;
        int sz;
        T* elem;
    public:
        iterator(int head, int tail, int post, int sz, T* elem) {
        ...
        bool operator==(persistent_queue<T>::iterator& other) {
    ...
    template <typename Container, typename T>
    void Engine<Container,T>::enqueueBatch() {
    }
};    
